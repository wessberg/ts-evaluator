...
export function evaluateFunctionExpression ({node, environment, evaluate, stack, ...rest}: IEvaluatorOptions<FunctionExpression>): Literal {
	...
	if (nameResult != null) setInLexicalEnvironment(environment, nameResult, functionExpression.bind(functionExpression));

	/**
	 * An implementation of the function expression
	 * @param {Literal} args
	 */
	...
	functionExpression.toString = () => `[Function${nameResult == null ? "" : `: ${nameResult}`}]`;
