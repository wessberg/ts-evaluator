...
export function evaluateFunctionDeclaration (options: IEvaluatorOptions<FunctionDeclaration>): void {
	...
	if (nameResult != null) setInLexicalEnvironment(environment, nameResult, functionDeclaration.bind(functionDeclaration));

	/**
	 * An implementation of the function declaration
	 * @param {Literal} args
	 */
	...
	functionDeclaration.toString = () => `[Function${nameResult == null ? "" : `: ${nameResult}`}]`;
	...
	stack.push(functionDeclaration);
}